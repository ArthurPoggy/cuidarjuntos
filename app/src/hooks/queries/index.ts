/**
 * Hooks de React Query — substituem o padrão `useEffect + useState + try/catch` espalhado pelas telas.
 *
 * Uso:
 *   const { data, isLoading, error } = useDashboard({ start, end });
 *   const { mutate: setStatus } = useSetRecordStatus();
 *
 * Cada hook lida com cache automaticamente; mutations invalidam queries relacionadas.
 */
