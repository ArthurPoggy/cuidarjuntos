...
// ---- Local types exported to screens ----
...
export type ChartKind = 'sleep' | 'medication' | 'meal' | 'vital' | 'activity' | 'progress' | 'bathroom';
...
export const groupsApi = {
  ...
  current: () =>
    client.get<{ group: CareGroup | null; membership?: { relation_to_patient: string } }>('/groups/current/'),
};
...
export const shiftsApi = {
  list: (params?: Record<string, string>) =>
    client.get<PaginatedResponse<CareShift>>('/shifts/', { params }),

  create: (data: Partial<CareShift> & Record<string, unknown>) =>
    client.post<CareShift>('/shifts/', data),

  delete: (id: number) => client.delete(`/shifts/${id}/`),

  deleteSeries: (id: number) => client.delete(`/shifts/${id}/delete_series/`),
};

// ---- Checklist ----
export const checklistApi = {
  list: (params?: Record<string, string>) =>
    client.get<PaginatedResponse<ChecklistItem>>('/checklist/', { params }),

  create: (data: Partial<ChecklistItem> & Record<string, unknown>) =>
    client.post<ChecklistItem>('/checklist/', data),

  delete: (id: number) => client.delete(`/checklist/${id}/`),

  toggle: (id: number) => client.post<ChecklistItem>(`/checklist/${id}/toggle/`),
};

// ---- Charts ----
export const chartsApi = {
  fetch: (kind: ChartKind, params?: Record<string, string>) =>
    client.get<ChartResponse>(`/charts/${kind}/`, { params }),
};
...
export const exportsApi = {
  ...
  download: (format: ExportFormat, params?: Record<string, string>) =>
    client.get(`/export/${format}/`, { params, responseType: 'blob' }),
};
...
export const notificationsApi = {
  ...
  // Retorna apenas a contagem de não lidas (via count do response paginado com page_size=1)
  unreadCount: () =>
